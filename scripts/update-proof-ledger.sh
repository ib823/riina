...
# Of the entries listed in _CoqProject, how many actually exist on disk
# (the buildable active set) and how many are stale references.
listed_on_disk_count=0
listed_missing_count=0
for rel in "${active_coq_files[@]}"; do
  if [ -f "$COQ_DIR/$rel" ]; then
    listed_on_disk_count=$((listed_on_disk_count + 1))
  else
    listed_missing_count=$((listed_missing_count + 1))
  fi
done

# Files present on disk but not declared in _CoqProject (orphan drafts,
# e.g. domains/_incomplete/*, work-in-progress NonInterference_v2*).
orphan_files_count=$((total_coq_files - listed_on_disk_count))
...
for rel in "${active_coq_files[@]}"; do
  ...
  grep -nE '^[[:space:]]*Axiom[[:space:]]+[A-Za-z_][A-Za-z0-9_]*[[:space:]]+:' "$path" | sed "s|^|$rel:|" >> "$tmp_active_axiom_sites" || true
...
global_axioms="$( { find "$REPO_ROOT/02_FORMAL" -name "*.v" -type f -exec grep -Eh '^[[:space:]]*Axiom[[:space:]]+[A-Za-z_][A-Za-z0-9_]*[[:space:]]+:' {} + 2>/dev/null || true; } | wc -l | tr -d " ")"
...
- Coq files (\`_CoqProject\` entries): $active_files_count
- Coq files (\`_CoqProject\` entries that exist on disk — the actual buildable set): $listed_on_disk_count
- Coq files (\`_CoqProject\` entries with no file on disk — stale references): $listed_missing_count
- Coq files (total .v files on disk): $total_coq_files
- Coq files (on disk but not in \`_CoqProject\` — orphan drafts): $orphan_files_count

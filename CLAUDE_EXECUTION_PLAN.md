# CLAUDE EXECUTION PLAN: ZERO ADMITS + ZERO AXIOMS (EXCLUDING _archive_deprecated)

## 0. Scope and Rules
- Scope includes all `02_FORMAL/coq/**/*.v` except `*/_archive_deprecated/*`.
- Goal: remove every `Admitted.` and `admit` in scope, and remove every `Axiom`/`Axioms`.
- No new axioms; proofs must align with current definitions.

## 1. Inventory Commands (use before/after)
```sh
find 02_FORMAL/coq -name '*.v' -not -path '*/_archive_deprecated/*' -print0 | xargs -0 grep -n 'Admitted\.'
find 02_FORMAL/coq -name '*.v' -not -path '*/_archive_deprecated/*' -print0 | xargs -0 grep -n -E '\\badmit\\b'
find 02_FORMAL/coq -name '*.v' -not -path '*/_archive_deprecated/*' -print0 | xargs -0 grep -n -E '^(Axiom|Axioms) '
```

## 2. Current Inventory (captured)
### 2.1 Admitted.
```
...
## 3. Work Order (strict)
1. **Semantics + Typing**
   - Confirm `step_deterministic` in `02_FORMAL/coq/foundations/Semantics.v` is usable; it replaces axioms in declassify proofs.
   - Confirm `substitution_preserves_typing` and `preservation` in `02_FORMAL/coq/type_system/Preservation.v` are complete.
   - Add bridge lemma: `Typing.store_wf Σ st -> (forall l v, store_lookup l st = Some v -> value v)` to satisfy SN_Closure.

2. **SN_Closure + ReducibilityFull (termination chain)**
   - `02_FORMAL/coq/properties/SN_Closure.v`
     - `SN_app_direct_aux` and `SN_app_direct_aux2`: remove admits by either (a) deleting unused lemmas, or (b) restating them to require `family_lambda_SN` and proving via `SN_app_family`. Ensure no admits remain.
   - `02_FORMAL/coq/termination/ReducibilityFull.v`
     - Prove `subst_subst_env_commute` by structural induction on `e`.
     - Close T_App case in `fundamental_reducibility` using `SN_app_family` + typing + preservation + substitution.
     - Close T_Deref case using the store_wf bridge.

3. **NonInterference_v2 (core step-up)**
   - `02_FORMAL/coq/properties/NonInterference_v2.v`
     - Finish `val_rel_at_type_step_up_with_IH` (store_rel step-up in TFn).
     - Complete `combined_step_up_all` using `preservation_store_wf` and `preservation_store_has_values`.
     - Prove `val_rel_at_type_TFn_step_0_bridge` using canonical forms + substitution_preserves_typing + well_typed_SN + multi_step_preservation.

4. **Fundamental theorem infrastructure**
   - `02_FORMAL/coq/properties/FundamentalTheorem.v`
     - Large batch of compat lemmas: complete by induction on typing derivations; use existing `val_rel`/`exp_rel` definitions and `substitution_preserves_typing`.
     - Ensure the final `fundamental_theorem` and corollaries are fully proved.

5. **Logical relation proof files**
   - `LogicalRelationDeclassify_v2.v`: replace `declassify_value_related` axiom with a proof using existing declassify relations and determinism.
   - `LogicalRelationDeclassify_PROOF.v`:
     - Replace axiom `step_deterministic` with `Semantics.step_deterministic`.
     - Replace `fundamental_lemma`/`subst_declassify`/`lc_declassify` etc. with actual lemmas (likely in `NonInterference_v2_LogicalRelation.v` or `Declassification.v`). If missing, prove in a shared helper file.
     - Close `exp_rel_n_step_closure` and `typing_declassify_inversion` admits.
   - `LogicalRelationDeclassify_PROOF_REFINED.v`:
     - Replace axioms with derived lemmas from other files (substitution distributivity, monotonicity, declassify eval).
     - Close remaining admits with explicit step-index reasoning.
   - `LogicalRelationAssign_PROOF.v`:
     - Replace `T_Loc`, `T_Assign` axioms with constructors from `Typing.v`.
     - Replace `val_rel_n_*` / `exp_rel_n_*` axioms with actual lemmas (likely in `NonInterference_v2_LogicalRelation.v` or `NonInterference_v2.v`).
     - Replace `fundamental_theorem` axiom with proved theorem from `FundamentalTheorem.v`.
   - `LogicalRelationRef_PROOF.v`:
     - Replace `store_ty_fresh_loc_none` axiom: prove from store typing + freshness lemma in `Semantics` or `StoreRelation`.
     - Close the remaining admit using fundamental lemma and store invariants.
   - `LogicalRelationDeref_PROOF_FINAL.v`:
     - Remove axiom `has_type` by importing `Typing.v`.
     - Replace store axioms (`store_contains_values`, `store_rel_same_domain`, `store_well_typed`) using `Typing.store_wf` and store relation lemmas in `StoreRelation.v`.
     - Replace `fundamental_lemma` and `fundamental_ref_produces_typed_loc` with actual theorems from `FundamentalTheorem.v` or derived corollaries.

6. **NonInterference logical relation (v2)**
   - `NonInterference_v2_LogicalRelation.v`: many admits around HO cases, closedness, and store_wf propagation.
     - Strategy: use `val_rel_n_step_up_from_combined`, `store_rel_n_step_up_from_combined` (from `NonInterference_v2.v`) plus `preservation_store_wf` to discharge typing/closedness obligations.
     - Replace axioms (`logical_relation_ref/deref/assign/declassify`, `val_rel_n_to_val_rel`) with the corresponding proved theorems from other files or prove them locally using the completed fundamental theorem.

7. **Kripke/Cumulative/Relation bridge**
   - `KripkeProperties.v`, `KripkeMutual.v`, `NonInterferenceKripke.v`, `CumulativeRelation.v`, `CumulativeMonotone.v`, `RelationBridge.v`, `ValRelStepLimit_PROOF.v`:
     - Replace admits by adding missing monotonicity/step-up lemmas; typically these are structural inductions on type or step index. Several admits mention “need stronger premise”; formalize that premise (e.g., `n > fo_compound_depth T`) and pass it through.

8. **Other proof files**
   - `ReferenceOps.v`, `AxiomEliminationVerified.v`, `ApplicationComplete.v`, `NonInterferenceZero.v`, `TypedConversion.v`, `MasterTheorem.v`, `Declassification.v`:
     - Close admits using: determinism (`Semantics.step_deterministic`), preservation, canonical forms, and the now-complete logical relations.
     - For `MasterTheorem.v`, use `step_preserves_closed`, store invariants, and step-up lemmas to replace the admitted cases.

9. **Industry axioms removal (Axioms = 0)**
   - Introduce a compliance model module in `02_FORMAL/coq/compliance/ComplianceModel.v`:
     - Define evidence records per standard (e.g., `IEC62443_Evidence`, `HIPAA_Evidence`).
     - Define `*_compliance` predicates in terms of evidence.
   - Update each `02_FORMAL/coq/Industries/*.v` file:
     - Replace `Axiom` with `Theorem` requiring explicit evidence parameter(s).
     - If any usage sites exist, update them to provide evidence or to accept the new hypothesis.

10. **Final verification**
   - Build Coq: `make` in `02_FORMAL/coq`.
   - Re-run inventory commands; ensure zero results for `Admitted.` and `Axiom`.

## 4. Per-file Action List (admit-heavy files)
- `02_FORMAL/coq/properties/SN_Closure.v`
  - Remove admits in `SN_app_direct_aux`/`SN_app_direct_aux2` by reworking to use `family_lambda_SN` or deleting unused lemmas.
- `02_FORMAL/coq/termination/ReducibilityFull.v`
  - Prove `subst_subst_env_commute` (structural induction).
  - Close `fundamental_reducibility` T_App and T_Deref cases.
- `02_FORMAL/coq/properties/NonInterference_v2.v`
  - Finish `val_rel_at_type_step_up_with_IH`, `combined_step_up_all`, `val_rel_at_type_TFn_step_0_bridge`.
- `02_FORMAL/coq/properties/FundamentalTheorem.v`
  - Complete all compatibility lemmas and the final fundamental theorem.
- `02_FORMAL/coq/properties/NonInterference_v2_LogicalRelation.v`
  - Systematically replace admits in HO cases using completed step-up + preservation lemmas.
- `02_FORMAL/coq/properties/LogicalRelationDeclassify_PROOF.v`
  - Replace axioms with existing proofs (determinism, fundamental lemma, monotonicity); close admits.
- `02_FORMAL/coq/properties/LogicalRelationDeclassify_PROOF_REFINED.v`
  - Replace axioms with proven lemmas; close admits.
- `02_FORMAL/coq/properties/LogicalRelationAssign_PROOF.v`
  - Replace typing axioms with constructors; replace relation axioms with proven lemmas.
- `02_FORMAL/coq/properties/LogicalRelationRef_PROOF.v`
  - Prove `store_ty_fresh_loc_none` and close the remaining admit.
- `02_FORMAL/coq/properties/LogicalRelationDeref_PROOF_FINAL.v`
  - Remove all axioms by importing actual definitions and proving store lemmas.
- `02_FORMAL/coq/properties/KripkeProperties.v`, `KripkeMutual.v`, `NonInterferenceKripke.v`, `CumulativeRelation.v`, `CumulativeMonotone.v`, `RelationBridge.v`
  - Fill structural and monotonicity admits (likely by induction on type + step index).
- `02_FORMAL/coq/properties/ReferenceOps.v`, `AxiomEliminationVerified.v`, `ApplicationComplete.v`, `NonInterferenceZero.v`, `TypedConversion.v`, `MasterTheorem.v`, `Declassification.v`
  - Replace admits using determinism + preservation + canonical forms + new step-up lemmas.

## 5. Completion Criteria
- `grep -R 'Admitted\.'` returns nothing.
- `grep -R -E '^(Axiom|Axioms) '` returns nothing.
- `make` under `02_FORMAL/coq` succeeds.

# DELEGATION PROMPT: TAU-001 MESH NETWORKING COQ PROOFS
...
```
=======================================================================================================
TASK ID: TAU-001-MESH-NETWORKING-COQ-PROOFS
...
This task implements proofs from:
  01_RESEARCH/44_DOMAIN_TAU_MESH_NETWORKING/RESEARCH_TAU01_FOUNDATION.md

Domain: Tau (Verified Mesh Networking)
Focus: Byzantine-tolerant ad-hoc network routing

Core Principle: "Route survives even with compromised nodes."

Key Properties:
- Byzantine fault-tolerant routing (3f+1)
...
You MUST output a SINGLE Coq file named `MeshNetworking.v` that:

1. Defines mesh network topology and routing model
2. Defines Byzantine fault tolerance for routing
3. Proves that 25 specific mesh networking properties hold
...
Proof.
  ...
  intro Hbyz.
  apply existsb_exists in Hbyz.
  - destruct Hbyz as [b [Hin' Heq]].
    apply Nat.eqb_eq in Heq. subst.
    rewrite existsb_exists in H.
    apply H. exists n. split; [exact Hin' | apply Nat.eqb_refl].
  - exists n. split; [exact Hbyz | apply Nat.eqb_refl].
Qed.
...
Proof.
  ...
  apply NoDup_nodup_length. exact H.
Qed.
...
Proof.
  ...
  - destruct Hin as [s [Hin' Heq]].
    apply Nat.eqb_eq in Heq. subst.
    rewrite existsb_exists in H.
    apply H. exists msg_id. split; [exact Hin' | apply Nat.eqb_refl].
  - exists msg_id. split; [exact Hin | apply Nat.eqb_refl].
Qed.
...
1. DO NOT change theorem names - use EXACTLY `mesh_001_*` through `mesh_025_*`
...
Your output will be saved to `MeshNetworking.v` and these checks will be run:

```bash
# Check 1: Must compile
coqc MeshNetworking.v
# Exit code MUST be 0

# Check 2: Count Admitted (must be 0)
grep -c "Admitted\." MeshNetworking.v
# MUST return 0

# Check 3: Count admit tactic (must be 0)
grep -c "admit\." MeshNetworking.v
# MUST return 0

# Check 4: Count theorems (must be 25)
grep -c "^Theorem mesh_" MeshNetworking.v
# MUST return 25

# Check 5: No new axioms
grep -c "^Axiom" MeshNetworking.v
# MUST return 0
```
...
START YOUR OUTPUT WITH `(* MeshNetworking.v *)` AND END WITH THE FINAL LINE OF THE FILE.

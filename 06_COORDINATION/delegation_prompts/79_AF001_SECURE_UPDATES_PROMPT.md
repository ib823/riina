# DELEGATION PROMPT: AF-001 VERIFIED SECURE UPDATES COQ PROOFS
...
```
=======================================================================================================
TASK ID: AF-001-SECURE-UPDATES-COQ-PROOFS
...
This task implements proofs from:
  01_RESEARCH/51_DOMAIN_AF_SECURE_UPDATES/RESEARCH_AF01_FOUNDATION.md

Domain: AF (Verified Secure Updates)
Focus: Cryptographically verified software update mechanism

Core Principle: "Every update signed. No rollback permitted."

Key Properties:
- Update signature verification
- Version monotonicity (anti-rollback)
...
You MUST output a SINGLE Coq file named `SecureUpdates.v` that:

1. Defines update package and verification model
2. Defines anti-rollback and atomic application
3. Proves that 25 specific update security properties hold
...
1. DO NOT change theorem names - use EXACTLY `update_001_*` through `update_025_*`
...
Your output will be saved to `SecureUpdates.v` and these checks will be run:

```bash
# Check 1: Must compile
coqc SecureUpdates.v
# Exit code MUST be 0

# Check 2: Count Admitted (must be 0)
grep -c "Admitted\." SecureUpdates.v
# MUST return 0

# Check 3: Count admit tactic (must be 0)
grep -c "admit\." SecureUpdates.v
# MUST return 0

# Check 4: Count theorems (must be 25)
grep -c "^Theorem update_" SecureUpdates.v
# MUST return 25

# Check 5: No new axioms
grep -c "^Axiom" SecureUpdates.v
# MUST return 0
```
...
START YOUR OUTPUT WITH `(* SecureUpdates.v *)` AND END WITH THE FINAL LINE OF THE FILE.

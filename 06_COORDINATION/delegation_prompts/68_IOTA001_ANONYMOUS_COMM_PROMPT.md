# DELEGATION PROMPT: IOTA-001 ANONYMOUS COMMUNICATION COQ PROOFS
...
```
=======================================================================================================
TASK ID: IOTA-001-ANONYMOUS-COMM-COQ-PROOFS
...
Domain: Iota (Anonymous Communication)
Focus: Cryptographically guaranteed sender/receiver anonymity

Core Principle: "No observer learns who talks to whom."

Key Properties:
- Sender anonymity (sender hidden from all)
- Receiver anonymity (receiver hidden from all)
- Sender-receiver unlinkability
- Global passive adversary resistance
- Onion routing correctness

=======================================================================================================
SECTION 2: WHAT YOU MUST PRODUCE
=======================================================================================================

You MUST output a SINGLE Coq file named `AnonymousComm.v` that:

1. Defines onion routing encryption layers
2. Defines anonymity sets and unlinkability relations
3. Proves that 25 specific anonymous communication properties hold
...
Definition rendezvous_hidden (rp_id observer_known : list nat) : Prop :=
...
1. DO NOT change theorem names - use EXACTLY `anon_001_*` through `anon_025_*`
...
Your output will be saved to `AnonymousComm.v` and these checks will be run:

```bash
# Check 1: Must compile
coqc AnonymousComm.v
# Exit code MUST be 0

# Check 2: Count Admitted (must be 0)
grep -c "Admitted\." AnonymousComm.v
# MUST return 0

# Check 3: Count admit tactic (must be 0)
grep -c "admit\." AnonymousComm.v
# MUST return 0

# Check 4: Count theorems (must be 25)
grep -c "^Theorem anon_" AnonymousComm.v
# MUST return 25

# Check 5: No new axioms
grep -c "^Axiom" AnonymousComm.v
# MUST return 0
```
...
START YOUR OUTPUT WITH `(* AnonymousComm.v *)` AND END WITH THE FINAL LINE OF THE FILE.

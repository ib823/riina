# DELEGATION PROMPT: PSI-001 OPERATIONAL SECURITY COQ PROOFS
...
```
=======================================================================================================
TASK ID: PSI-001-OPERATIONAL-SECURITY-COQ-PROOFS
...
This task implements proofs from:
  01_RESEARCH/31_DOMAIN_PSI_OPERATIONAL_SECURITY/RESEARCH_PSI01_FOUNDATION.md

Domain: Psi (Operational Security)
Focus: Multi-party security, threshold cryptography, insider threat mitigation

Core Principle: "Trust no single human. Trust the protocol."

=======================================================================================================
SECTION 2: WHAT YOU MUST PRODUCE
=======================================================================================================

You MUST output a SINGLE Coq file named `OperationalSecurity.v` that:

1. Defines a threshold cryptography model (Shamir Secret Sharing)
2. Defines multi-party authorization protocols
3. Proves that 25 specific operational security properties hold
...
Proof.
  ...
  apply NoDup_length_nodup. exact H_nodup.
Qed.
...
Definition is_duress (input : list nat) (duress_suffix : list nat) : bool :=
  ...
  list_eq_dec Nat.eq_dec suffix duress_suffix.
...
1. DO NOT change theorem names - use EXACTLY `opsec_001_*` through `opsec_025_*`
...
Your output will be saved to `OperationalSecurity.v` and these checks will be run:

```bash
# Check 1: Must compile
coqc OperationalSecurity.v
# Exit code MUST be 0

# Check 2: Count Admitted (must be 0)
grep -c "Admitted\." OperationalSecurity.v
# MUST return 0

# Check 3: Count admit tactic (must be 0)
grep -c "admit\." OperationalSecurity.v
# MUST return 0

# Check 4: Count theorems (must be 25)
grep -c "^Theorem opsec_" OperationalSecurity.v
# MUST return 25

# Check 5: No new axioms
grep -c "^Axiom" OperationalSecurity.v
# MUST return 0
```
...
START YOUR OUTPUT WITH `(* OperationalSecurity.v *)` AND END WITH THE FINAL LINE OF THE FILE.

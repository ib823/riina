# DELEGATION PROMPT: RHO-001 VERIFIED AUTONOMY COQ PROOFS
...
```
=======================================================================================================
TASK ID: RHO-001-VERIFIED-AUTONOMY-COQ-PROOFS
...
This task implements proofs from:
  01_RESEARCH/40_DOMAIN_RHO_VERIFIED_AUTONOMY/RESEARCH_RHO01_FOUNDATION.md

Domain: Rho (Verified Autonomy)
Focus: Formally verified autonomous system decision-making

Core Principle: "Every autonomous decision provably safe."
...
You MUST output a SINGLE Coq file named `VerifiedAutonomy.v` that:

1. Defines safety envelope and decision models
2. Defines failsafe and override mechanisms
3. Proves that 25 specific autonomous system properties hold
...
Proof.
  ...
  intro Hobs.
  apply existsb_exists in Hobs.
  - destruct Hobs as [o [Hin' Heq]].
    apply Nat.eqb_eq in Heq. subst.
    rewrite existsb_exists in H.
    apply H. exists p. split; [exact Hin' | apply Nat.eqb_refl].
  - exists p. split; [exact Hobs | apply Nat.eqb_refl].
Qed.
...
1. DO NOT change theorem names - use EXACTLY `auto_001_*` through `auto_025_*`
...
Your output will be saved to `VerifiedAutonomy.v` and these checks will be run:

```bash
# Check 1: Must compile
coqc VerifiedAutonomy.v
# Exit code MUST be 0

# Check 2: Count Admitted (must be 0)
grep -c "Admitted\." VerifiedAutonomy.v
# MUST return 0

# Check 3: Count admit tactic (must be 0)
grep -c "admit\." VerifiedAutonomy.v
# MUST return 0

# Check 4: Count theorems (must be 25)
grep -c "^Theorem auto_" VerifiedAutonomy.v
# MUST return 25

# Check 5: No new axioms
grep -c "^Axiom" VerifiedAutonomy.v
# MUST return 0
```
...
START YOUR OUTPUT WITH `(* VerifiedAutonomy.v *)` AND END WITH THE FINAL LINE OF THE FILE.

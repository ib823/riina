# DELEGATION PROMPT: WP-003 INJECTION PREVENTION COQ PROOFS
...
```
═══════════════════════════════════════════════════════════════════════════════════════════════════
TASK ID: WP-003-INJECTION-PREVENTION-COQ-PROOFS
CLASSIFICATION: FORMAL PROOF GENERATION — COQ PROOF ASSISTANT
SECURITY LEVEL: CRITICAL — NO ERRORS PERMITTED
MODE: ULTRA KIASU | FUCKING PARANOID | ZERO TRUST | INFINITE TIMELINE
═══════════════════════════════════════════════════════════════════════════════════════════════════

YOU ARE GENERATING A COMPLETE COQ PROOF FILE FOR INJECTION ATTACK PREVENTION.

READ EVERY WORD. FOLLOW EVERY INSTRUCTION EXACTLY. ANY DEVIATION = REJECTION.

═══════════════════════════════════════════════════════════════════════════════════════════════════
SECTION 1: WHAT YOU MUST PRODUCE
═══════════════════════════════════════════════════════════════════════════════════════════════════

Output a SINGLE Coq file named `InjectionPrevention.v` that:
1. Defines a taint tracking model for injection prevention
2. Proves that 15 injection attacks (INJ-001 through INJ-015) are IMPOSSIBLE
3. Contains ZERO `Admitted.` and ZERO `admit.`
4. Contains ZERO new `Axiom` declarations
5. Compiles with `coqc`

═══════════════════════════════════════════════════════════════════════════════════════════════════
SECTION 2: EXACT FILE CONTENT — YOU MUST OUTPUT THIS STRUCTURE
═══════════════════════════════════════════════════════════════════════════════════════════════════
...
Proof.
  ...
  - destruct (Nat.eqb c 61 || Nat.eqb c 43 || Nat.eqb c 45 || Nat.eqb c 64) eqn:E.
    + simpl. trivial.  (* Starts with quote now *)
    + (* Original character - check it's not dangerous *)
      apply Bool.orb_false_iff in E.
      destruct E as [E1 E2].
      apply Bool.orb_false_iff in E1.
      destruct E1 as [E1a E1b].
      apply Bool.orb_false_iff in E1a.
      destruct E1a as [E1a1 E1a2].
      apply Nat.eqb_neq in E1a1.
      apply Nat.eqb_neq in E1a2.
      apply Nat.eqb_neq in E1b.
      apply Nat.eqb_neq in E2.
      destruct c; trivial.
      destruct c; trivial.
      (* ... pattern continues - all safe cases *)
      trivial.
Qed.
...
═══════════════════════════════════════════════════════════════════════════════════════════════════
SECTION 3: FORBIDDEN ACTIONS — VIOLATION = REJECTION
═══════════════════════════════════════════════════════════════════════════════════════════════════

1. NO `Admitted.`
2. NO `admit.`
3. NO new `Axiom` or `Parameter`
4. EXACT names: `inj_001_*` through `inj_015_*`
5. NO markdown around output
6. NO text before or after Coq code

═══════════════════════════════════════════════════════════════════════════════════════════════════
SECTION 4: OUTPUT
═══════════════════════════════════════════════════════════════════════════════════════════════════

OUTPUT ONLY THE COQ FILE. START WITH `(* InjectionPrevention.v *)`. BEGIN NOW:
```

...
//! - TERAS-LANG bootstrap compilation
...
#[derive(Parser)]
#[command(name = "teras-build")]
#[command(version = "1.0.0")]
#[command(about = "TERAS Build System - Hermetic, Reproducible, Verified")]
...
This tool orchestrates builds across all TERAS components:
- Rust crates (with verification tool integration)
- Ada/SPARK packages (with GNATprove)
- TERAS-LANG compiler (bootstrap stages)
...
struct Cli {
    ...
    /// Project root (default: current directory or TERAS_ROOT)
...
impl BuildContext {
    fn new(cli: &Cli) -> Result<Self, BuildError> {
        ...
        // Verify we're in a TERAS project
        if !root.join("Cargo.toml").exists() {
            return Err(BuildError::NotTerasProject(root));
        }
...
impl std::fmt::Display for BuildError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BuildError::NotTerasProject(path) => {
                write!(f, "Not a TERAS project: {}", path.display())
            }
...
fn build_bootstrap(ctx: &BuildContext, stage: u8, verify: bool) -> Result<(), BuildError> {
    ctx.log(&format!("Bootstrapping TERAS-LANG (stage {stage})..."));
    ...
    match stage {
        ...
        1 => {
            ...
            let stage0 = ctx.root.join("target/release/terasc");
            if !stage0.exists() {
                return Err(BuildError::InvalidConfiguration(
                    "Stage 0 compiler not found. Run `teras-build bootstrap --stage 0` first.".to_string()
                ));
            }

            // TODO: Actually compile with stage 0 when TERAS-LANG is ready
        ...
        2 => {
            ...
            let stage1 = ctx.root.join("target/release/terasc");
            if !stage1.exists() {
                return Err(BuildError::InvalidConfiguration(
                    "Stage 1 compiler not found. Run `teras-build bootstrap --stage 1` first.".to_string()
                ));
            }
...
fn show_config(ctx: &BuildContext) {
    println!("═══════════════════════════════════════════════════════════════");
    println!("                    TERAS BUILD CONFIGURATION");
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    ...
    println!("  bootstrap  - TERAS-LANG compiler stages");
...
fn main() -> ExitCode {
    ...
    println!("═══════════════════════════════════════════════════════════════");
    println!("           TERAS BUILD SYSTEM v1.0.0");

...
fun readFile(path: string) : âŸ¨io, exn | ÎµâŸ© string
```
...
**Mechanism:**
```
Effect subtyping: Îµâ‚ âŠ† Îµâ‚‚ implies (Ï„ â†’^Îµâ‚ Ïƒ) â‰¤ (Ï„ â†’^Îµâ‚‚ Ïƒ)
```
...
f : Int â†’^{read} Int
g : (Int â†’^{read,write} Int) â†’ Int
g(f)  // OK: {read} âŠ† {read,write}
```
...
**Mechanism:**
```
Effect rows: âŸ¨lâ‚ | âŸ¨lâ‚‚ | ÎµâŸ©âŸ© where Îµ is variable
```

**Example:**
```koka
// Effect variable captures "other effects"
fun foo(f : () -> âŸ¨exn | ÎµâŸ© int) : âŸ¨exn | ÎµâŸ© int
  f()
```
...
sandbox : Eff âŸ¨FileOp, NetOp | ÎµâŸ© Î± â†’ Eff Îµ (Option Î±)
```

**Linear Capabilities (from A-04):**
```teras
type Capability<R> = lin { cap: Token | authorized(cap) }
use_cap : Capability<R> âŠ¸ R â†’ (Result, Option<Capability<R>>)
```

**Combined:**
```teras
effect UseCapability<R> : Capability<R> -> (R -> Eff Îµ Î±) -> Î±
```
...
handle_use : HandlerâŸ¨UseâŸ© â†’ (lin R) â†’ Eff Îµ Result
```

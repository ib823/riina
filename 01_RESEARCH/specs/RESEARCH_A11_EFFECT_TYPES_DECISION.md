...
### Session A-11: Effect Types â€” Architecture Decision
...
```
// Row extension
<State | <IO | Îµ>> = <State, IO | Îµ>

// Row unification
<eâ‚ | Ïâ‚> ~ <eâ‚‚ | Ïâ‚‚>  âŸ¹  eâ‚ = eâ‚‚ âˆ§ Ïâ‚ ~ Ïâ‚‚
                        âˆ¨  âˆƒÏ'. Ïâ‚ = <eâ‚‚ | Ï'> âˆ§ <eâ‚ | Ï'> ~ Ïâ‚‚
                        âˆ¨  âˆƒÏ'. Ïâ‚‚ = <eâ‚ | Ï'> âˆ§ Ïâ‚ ~ <eâ‚‚ | Ï'>
...
```
Input: Expression e, Environment Î“
Output: (Type Ï„, Effect Îµ, Substitution Î¸)

infer(Î“, e) = match e with
    | x -> 
        let Ï„ = Î“(x)
        (instantiate(Ï„), <>, id)
    
    | Î»x. e' ->
        let Î±, Î², Ï = fresh_vars()
        let (Ï„, Îµ, Î¸) = infer(Î“[x â†¦ Î±], e')
        (Î¸(Î±) -> Ï„ / Îµ, <>, Î¸)
    
    | eâ‚ eâ‚‚ ->
        let (Ï„â‚, Îµâ‚, Î¸â‚) = infer(Î“, eâ‚)
        let (Ï„â‚‚, Îµâ‚‚, Î¸â‚‚) = infer(Î¸â‚(Î“), eâ‚‚)
        let Î±, Ï = fresh_vars()
        let Î¸â‚ƒ = unify(Î¸â‚‚(Ï„â‚), Ï„â‚‚ -> Î± / Ï)
        (Î¸â‚ƒ(Î±), Î¸â‚ƒ(Îµâ‚) âˆª Î¸â‚ƒ(Îµâ‚‚) âˆª Î¸â‚ƒ(Ï), Î¸â‚ƒ âˆ˜ Î¸â‚‚ âˆ˜ Î¸â‚)
    
    | perform op(e') ->
        let E = effect_of(op)
        let (Ï„_arg, Ï„_ret) = signature_of(op)
        let (Ï„', Îµ, Î¸) = infer(Î“, e')
        let Î¸' = unify(Ï„', Ï„_arg)
        (Î¸'(Ï„_ret), Î¸'(Îµ) âˆª <E>, Î¸' âˆ˜ Î¸)
    
    | handle e' with h ->
        let E = handled_effect(h)
        let (Ï„, Îµ, Î¸) = infer(Î“, e')
        let Îµ' = remove_effect(E, Î¸(Îµ))
        let Ï„' = handler_return_type(h, Ï„)
        (Ï„', Îµ', Î¸)
```
...
```
unify_row(Ïâ‚, Ïâ‚‚) = match (Ïâ‚, Ïâ‚‚) with
    | (<>, <>) -> id
    | (Î±, Ï) -> [Î± â†¦ Ï] if Î± âˆ‰ fv(Ï)
    | (Ï, Î±) -> [Î± â†¦ Ï] if Î± âˆ‰ fv(Ï)
    | (<e | Ïâ‚'>, <e | Ïâ‚‚'>) -> unify_row(Ïâ‚', Ïâ‚‚')
    | (<eâ‚ | Ïâ‚'>, <eâ‚‚ | Ïâ‚‚'>) when eâ‚ â‰  eâ‚‚ ->
        let Î² = fresh_var()
        let Î¸â‚ = unify_row(Ïâ‚', <eâ‚‚ | Î²>)
        let Î¸â‚‚ = unify_row(Î¸â‚(Ïâ‚‚'), <eâ‚ | Î¸â‚(Î²)>)
        Î¸â‚‚ âˆ˜ Î¸â‚
```
...
|-----|------------|-------|
| Self-sufficiency | âœ“ | No external dependencies |
| Formal verification | âœ“ | Row types well-understood |
| Performance mandate | âœ“ | Evidence passing efficient |
| Security guarantees | âœ“ | Capability-based security effects |

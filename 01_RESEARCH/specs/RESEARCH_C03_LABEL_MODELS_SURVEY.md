...
Label = Level Ã— Compartments Ã— Integrity
...
Label = { ownerâ‚: readersâ‚; ownerâ‚‚: readersâ‚‚; ... }
...
Example:
    ...
    Effective readers = {Bob, Charlie} âˆ© {Eve} = {}
```
...
Join (most restrictive):
    {o: Râ‚} âŠ” {o: Râ‚‚} = {o: Râ‚ âˆ© Râ‚‚}
    
    "Only principals in BOTH sets can read"

Meet (least restrictive):
    {o: Râ‚} âŠ“ {o: Râ‚‚} = {o: Râ‚ âˆª Râ‚‚}
    
    "Principals in EITHER set can read"

Flow relation:
    Lâ‚ âŠ‘ Lâ‚‚ iff effective_readers(Lâ‚‚) âŠ† effective_readers(Lâ‚)
    
    "Lâ‚‚ is more restrictive than Lâ‚"
```
...
Type: (u: User) â†’ Data @ u.clearance
```
...
policy UserCanRead(user: User, data: Data) {
    user.clearance âŠ’ data.classification
}
...
fn upgrade<L1, L2>(x: T @ L1, target: L2) -> T @ (L1 âŠ” L2)
    where L1 âŠ‘ L2
{
    x as T @ (L1 âŠ” L2)
}
...
Flow direction (OPPOSITE to confidentiality):
    ...
    Untrusted â†’ Trusted   âœ— (blocked)
```
...
Label = { owners : readers ; â† writers }
        ^^^^^^^^^^^^^^^^^^^   ^^^^^^^^^^
        Confidentiality       Integrity

Integrity meaning:
    { â† Alice, Bob }  means Alice and Bob have "endorsed" this
...
fn internal() {
    ...
    publish(b)            // ERROR: Secret â†’ Public
}
```
...
impl Label {
    ...
    pub fn can_flow_to(&self, target: &Label) -> bool {
        // Confidentiality: self must be â‰¤ target
        self.level <= target.level &&
        self.compartments.is_subset(&target.compartments) &&
        // Integrity: self must be â‰¥ target (opposite direction)

...
```
Data Races:
    Thread 1: read x â†’ compute â†’ write x
    Thread 2: read x â†’ compute â†’ write x
...
// Relationships
// T: Sync âŸ¹ &T: Send
// T: Send + Copy âŸ¹ T: Sync (for most types)
```
...
impl<T, L: Label> LabeledChannel<T, L> {
    fn send(&self, value: T) -> Result<(), SendError<T>>
    where
        CurrentPC: FlowsTo<L>  // Can only send if PC â‰¤ Label
    {
...
*Research Track Output â€” Domain H: Concurrency and Parallelism*

...
Linear Logic Origins:
  A âŠ¸ B      (linear implication: use A exactly once)
  A âŠ— B      (tensor: both A and B)
  A & B      (with: choose A or B)
  A âŠ• B      (plus: offer A or B)
  
Resource Interpretation:
  A âŠ¸ B means: consuming A produces B
...
Syntax:
  ptr(Ï„, l)     -- pointer to Ï„ at location l
  lâ‚ = lâ‚‚      -- locations may alias
  lâ‚ â‰  lâ‚‚      -- locations disjoint
```

### 1.1.3 Adoption and Focus (FÃ¤hndrich & DeLine, 2002)
...
Ownership â‰ˆ Affine Types + Deallocation
  - Affine: at most once
...
// 2. Single input â†’ output gets that lifetime
// 3. &self â†’ output gets self's lifetime
...
Mezzo is a programming language with advanced ownership types, developed by FranÃ§ois Pottier and Jonathan Protzenko at INRIA (2013-2017).
...
val p: (x @ point) â†’ duplicable int
...
val swap_children: (t: tree a) â†’ unit =
  focus t as (
    | Leaf â†’ ()
    | Node { left; right; _ } â†’
        let tmp = left in
...
(* Mode inference *)
val id: [a] a â†’ a  (* Polymorphic over modes *)
```
...
Structural Rules (restricted):
  Weakening: Î“, x:Ï„ âŠ¢ e : Ïƒ   â†’   Î“ âŠ¢ e : Ïƒ     (allowed)
  Contraction: Î“, x:Ï„, y:Ï„ âŠ¢ e : Ïƒ   â†’   Î“, z:Ï„ âŠ¢ e[z/x,z/y] : Ïƒ   (forbidden)
  
Affine Types:
  Ï„ ::= A                -- affine type
      | Ï„â‚ â†’â‚ Ï„â‚‚        -- affine function
      | Ï„â‚ âŠ— Ï„â‚‚         -- tensor (pair)
      | !Ï„              -- duplicable (exponential)

Typing:
  Î“ âŠ¢ e : Ï„    where Î“ is affine context
  
  Each variable in Î“ used at most once
```
...
Ownership as Separation:
  x â†¦ v  (x points to v, exclusive)
  
  x â†¦ v * y â†¦ w  (x and y point to different locations)
...
Theorem (Progress):
  If Â· âŠ¢ e : Ï„ then either:
  - e is a value, or
  - âˆƒe'. e â†’ e'

Theorem (Preservation):
  If Î“ âŠ¢ e : Ï„ and e â†’ e'
  then Î“' âŠ¢ e' : Ï„ for some Î“' âŠ† Î“
...
Ownership specializes:
  owned T   â‰ˆ aff T + Drop
  &T        â‰ˆ shared borrow
  &mut T    â‰ˆ unique borrow
...
Combined:
  ...
  Meaning:
    ...
    - Allocated in region Ï
    - Deallocated when Ï ends or owner drops

Reference into region:
  &'Ï T      -- borrow from region Ï
  &'Ï mut T  -- mutable borrow from Ï
```
...
7. FÃ¤hndrich, M., & DeLine, R. (2002). "Adoption and Focus: Practical Linear Types for Imperative Programming." PLDI.

...
```
Static IFC:
â”œâ”€â”€ Enforced at compile time
â”œâ”€â”€ No runtime overhead
â”œâ”€â”€ Conservative (rejects some safe programs)
â”œâ”€â”€ Full coverage guarantee
â””â”€â”€ Cannot handle runtime-determined labels

Dynamic IFC:
â”œâ”€â”€ Enforced at runtime
â”œâ”€â”€ Runtime overhead
â”œâ”€â”€ Precise (accepts safe executions)
â”œâ”€â”€ May miss paths
â””â”€â”€ Handles dynamic labels naturally
```
...
x = read_secret()       // current_label â†’ Secret
y = x + 1               // Computation taints result
output(y)               // Check: current_label âŠ‘ output_label
                        // Fails: Secret â‹¢ Public
```
...
Read:  check(current_label âŠ’ ref_label)
Write: check(current_label âŠ‘ ref_label)
...
-- setLabel must satisfy: new_label âŠ‘ clearance
...
fn dynamic_flow(x: int @ ?L, target: Label) -> int @ target {
    runtime_check!(L âŠ‘ target);  // Runtime check
    x
}
```

...
Levels:
â”œâ”€â”€ Type checking: Lightweight verification
â”œâ”€â”€ Static analysis: Approximation-based
â”œâ”€â”€ Model checking: Exhaustive state exploration
â”œâ”€â”€ Theorem proving: Full mathematical proof
â””â”€â”€ Certified compilation: Proof-preserving translation
```
...
// x â†¦ v * y â†¦ w means x and y are separate

// Exclusive access
#[requires(file â†¦ contents)]
#[ensures(file â†¦ new_contents)]
fn write_exclusive(file: &mut File, new_contents: Bytes) { ... }

// Shared reading
#[requires(file â†¦ contents)]
#[ensures(file â†¦ contents)]  // Unchanged
...
â”œâ”€â”€ Coq: Dependent types, extraction
â”œâ”€â”€ Isabelle/HOL: Higher-order logic
â”œâ”€â”€ Lean: Modern, good automation
â”œâ”€â”€ Agda: Dependently typed language
â””â”€â”€ F*: Effect-oriented verification
```
...
Passes:
    C â†’ Clight â†’ C#minor â†’ Cminor â†’ CminorSel â†’
    RTL â†’ LTL â†’ Linear â†’ Mach â†’ Assembly

Each pass proven:
    âˆ€P. compile(P) = M â†’ P â‰ƒ M
```
...
// Type preservation
#[ensures(âˆ€e. type_of(e) = type_of(compile(e)))]

// Effect preservation  
#[ensures(âˆ€e. effects(e) = effects(compile(e)))]

// Security preservation
#[ensures(âˆ€e. secure(e) => secure(compile(e)))]

// Noninterference preservation
#[ensures(âˆ€P. noninterferent(P) => noninterferent(compile(P)))]
```
...
wp(while b do c, Q) = I âˆ§ âˆ€.(I âˆ§ b â‡’ wp(c, I)) âˆ§ (I âˆ§ Â¬b â‡’ Q)
```
...
*Research Track Output â€” Domain E: Formal Verification*

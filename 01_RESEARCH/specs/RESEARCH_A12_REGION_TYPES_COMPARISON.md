...
### Session A-12: Region Types â€” Comparative Analysis
...
|---------|-------|---------|------|-------|------|----------------|
| **Static Safety** | âœ“ Full | âœ“ Full | âœ— Runtime | âœ“ Full | âœ“ Full | âœ“ Full |
| **Region Inference** | âœ“ Full | âœ— Manual | âœ— None | Partial | Partial | âœ“ Full |
| **Effect Tracking** | âœ“ | Partial | âœ— | âœ“ | âœ— | âœ“ |
| **Region Polymorphism** | âœ“ | âœ“ | âœ— | âœ“ | âœ“ | âœ“ |
| **Unique Pointers** | âœ— | âœ“ | âœ— | âœ“ | âœ“ | âœ“ |
| **Adoption/Focus** | âœ— | âœ— | âœ— | âœ“ | âœ— | âœ— |
| **GC Integration** | âœ“ | âœ— | âœ“ | âœ— | âœ— | âœ— |
| **Bounds Checking** | Separate | âœ“ | Runtime | âœ“ | âœ“ | Separate |
| **C Compatibility** | âœ— | âœ“ High | N/A | âœ“ | Via FFI | âœ— |
| **Production Use** | âœ“ | âœ— Ended | âœ“ | âœ— Research | âœ“ | âœ— Research |
...
| No data races | N/A | âœ— | Runtime | Static | Static |
...
Expressiveness vs. Ease of Use:
    ...
    (full inference)  |
                      ...
                      |                 * Vault
                      |                   (adoption, focus)
    â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€> EXPRESSIVENESS
                      |
                      |
...
(* Inferred type:
   âˆ€Î±,Î²,Ïâ‚,Ïâ‚‚,Îµ.
   (Î± â†’^Îµ Î²) â†’ list(Î±,Ïâ‚) â†’^{Îµ,put(Ïâ‚‚)} list(Î²,Ïâ‚‚)
*)
```
...
|--------|-----------------|-------------|
| MLKit | get(Ï), put(Ï) | Per region |
| Cyclone | Implicit | Per access |
| Rust | None explicit | Via borrow rules |
| TERAS (proposed) | Read<Ï>, Write<Ï>, Alloc<Ï> | Per region |
...
Effect composition:
  fn f(x: &Ï„ at Ï) -> Ï„' ! {Read<Ï>}
  fn g(x: &mut Ï„ at Ï) -> () ! {Write<Ï>}
  fn h() -> Ï„ at Ï ! {Alloc<Ï>}

Effect masking at region boundary:
  letregion Ï in e end : Ï„ ! Îµ \ {Read<Ï>, Write<Ï>, Alloc<Ï>}
```
...
Security Attributes:
  letregion Ï [secret] in ...     -- secret data region
  letregion Ï [tainted] in ...    -- untrusted data region
  letregion Ï [ct] in ...         -- constant-time region

Type Integration:
  Secret<Ï„> at Ï[secret]          -- secret in secret region
  Tainted<Ï„> at Ï[tainted]        -- tainted in tainted region

Interaction Rules:
  Cannot flow: Ï[secret] â†’ Ï[public]
  Must sanitize: Ï[tainted] â†’ Ï[clean]
  Constant-time: Ï[ct] requires CT operations
```
...
Cyclone:
  ...
  - Fat pointer: 2Ã— pointer size
  - Tagged unions: 1 word tag
  
Rust:
  - No region overhead (static)
  - slice: 2Ã— pointer (ptr + len)
  - Box: pointer only
...
1. **Static safety is mandatory** â€” Runtime approaches (RTSJ) rejected for TERAS
2. **Inference is valuable** â€” MLKit approach reduces annotation burden
3. **Linear integration critical** â€” Fluet-Morrisett capability model ideal
4. **Security features needed** â€” Region attributes for confidentiality
5. **Rust validates approach** â€” Ownership/lifetimes prove practical viability
...
Type: lin Ï„ at Ï[attrs]
Effect: Read<Ï>, Write<Ï>, Alloc<Ï>

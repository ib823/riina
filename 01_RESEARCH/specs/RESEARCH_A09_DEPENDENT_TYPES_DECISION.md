...
Types:
  A, B ::= X                           -- Type variable
         | (q x : A) â†’ B               -- Quantitative dependent function
         | (x : A) Ã— B                 -- Dependent pair
         | {x : A | Ï†}                 -- Refinement type
         | A @ (q, u)                  -- Graded type (quantity, uniqueness)
         | Chan<S>                     -- Session channel
         | eff[E] A                    -- Effectful computation
         | âˆ€(x : A). B                 -- Universe-polymorphic
         | A â‰¡ B                       -- Propositional equality
```
...
(Ï‰ x: A) -> B     -- Unrestricted argument (use any times)

-- Formation rule
Î“ âŠ¢ A : Type_i    Î“, x:A âŠ¢ B : Type_j
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
     Î“ âŠ¢ (q x: A) â†’ B : Type_{max(i,j)}

-- Introduction rule
    Î“, qÂ·x:A âŠ¢ t : B
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
Î“ âŠ¢ Î»x.t : (q x: A) â†’ B

-- Elimination rule
Î“ âŠ¢ f : (q x: A) â†’ B    Î“ âŠ¢ qÂ·a : A
...
```
-- Dependent pair
type Sigma(x: A) Ã— B(x)

-- Syntax
(x: A) Ã— B        -- dependent pair type
(a, b)            -- pair introduction (a: A, b: B[a/x])

-- Formation rule
Î“ âŠ¢ A : Type_i    Î“, x:A âŠ¢ B : Type_j
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
     Î“ âŠ¢ (x: A) Ã— B : Type_{max(i,j)}
...
-- Notation: x â‰¡ y  for  Eq<A>(x, y)

-- Symmetry
fn sym<A: Type, x y: A>(p: x â‰¡ y) -> y â‰¡ x {
  match p {
    Refl => Refl
  }
}
...
-- Transitivity
fn trans<A: Type, x y z: A>(p: x â‰¡ y, q: y â‰¡ z) -> x â‰¡ z {
  match (p, q) {
    (Refl, Refl) => Refl
  }
}
...
-- Congruence
fn cong<A B: Type, x y: A>(f: A -> B, p: x â‰¡ y) -> f(x) â‰¡ f(y) {
  match p {
    Refl => Refl
  }
}
...
type LinPair<A, B> = (1 x: A) Ã— (1 _: B)
...
-- Inference: term â†’ type
infer(Î“, e) : (Type, Substitution)

-- Checking: term Ã— type â†’ success/failure
check(Î“, e, A) : Result<(), TypeError>

-- Core rules
infer(Î“, x) = lookup(Î“, x)
infer(Î“, f e) = 
  let (Î (x:A).B, Ïƒâ‚) = infer(Î“, f)
  let Ïƒâ‚‚ = check(Î“, e, A[Ïƒâ‚])
  (B[e/x][Ïƒâ‚âˆ˜Ïƒâ‚‚], Ïƒâ‚âˆ˜Ïƒâ‚‚)

check(Î“, Î»x.t, Î (x:A).B) =
  check(Î“, x:A, t, B)
...
A â‰¡ B  iff  normalize(A) = normalize(B)

-- Normalization rules
(Î»x.t) a âŸ¶ t[a/x]           -- Î²-reduction
(Ï€â‚ (a, b)) âŸ¶ a             -- pair projection
(Ï€â‚‚ (a, b)) âŸ¶ b
match Cons(x,xs) { Cons(y,ys) => e, ... } âŸ¶ e[x/y, xs/ys]

-- Type-directed Î·-expansion for comparison
normalize((Î»x.f x)) = normalize(f)  -- if x âˆ‰ fv(f)
```
...
infer_level(Î“, Type) = (Typeâ‚, level + 1)
infer_level(Î“, Prop) = (Typeâ‚€, 0)

-- Universe constraint solving
Î“ âŠ¢ A : Type_i    Î“ âŠ¢ B : Type_j
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
Î“ âŠ¢ A â†’ B : Type_{max(i, j)}

-- Predicativity check
Î“ âŠ¢ Î (x:A).B : Type_n
  requires: level(A) â‰¤ n  and  level(B) â‰¤ n
```
...
Î“ = (0Â·x:A, 1Â·y:B, Ï‰Â·z:C)

-- Split context for application
Î“ âŠ¢ f : (1 x:A) â†’ B    Î” âŠ¢ a : A
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        Î“ + Î” âŠ¢ f a : B

-- Context addition
0Â·Î“ + 0Â·Î“ = 0Â·Î“
qÂ·Î“ + 0Â·Î” = qÂ·Î“    (if Î“ and Î” agree on types)
1Â·x + 1Â·x = undefined (linear used twice)
```
...
fn label_join(l1: Label, l2: Label) -> Label {
  { level: max(l1.level, l2.level),
    compartments: l1.compartments âˆª l2.compartments }
}
...
|-------|----------|--------------|
| **Phase 1** | Weeks 1-6 | Core dependent types (Î , Î£) |
...
**Deliverables:**
1. Dependent function type (`Î `)
2. Dependent pair type (`Î£`)
3. Basic type inference
4. Î²-reduction and Î·-expansion

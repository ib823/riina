...
A monad (T, Î·, Î¼) consists of:
- Type constructor T : Type â†’ Type
- Unit/return: Î· : A â†’ T A
- Join/bind: Î¼ : T (T A) â†’ T A

Satisfying:
- Î¼ âˆ˜ T Î¼ = Î¼ âˆ˜ Î¼_T       (associativity)
- Î¼ âˆ˜ Î·_T = Î¼ âˆ˜ T Î· = id  (unit laws)
```
...
```
Swierstra: "Data Types Ã  la Carte" (2008)
...
```
1989    â”‚ Moggi: Computational monads
1991    â”‚ Moggi: "Notions of Computation and Monads"
1992    â”‚ Wadler: Comprehending Monads
1995    â”‚ Monad transformers introduced
1998    â”‚ mtl library for Haskell
2008    â”‚ Data Types Ã  la Carte
2013    â”‚ extensible-effects library
2015    â”‚ Freer monads paper
2017    â”‚ fused-effects library
2019    â”‚ polysemy library
2022    â”‚ effectful library
2024    â”‚ Continued refinement
```
...
-- The nÂ² instances problem
-- For n transformers, need nÂ² instances to allow any transformer
...
â”œâ”€â”€ Each layer adds indirection
â”œâ”€â”€ Function calls not inlined
â”œâ”€â”€ GHC can sometimes optimize, but not always
â””â”€â”€ Deep stacks = significant slowdown
...
```
Free monad overhead:
â”œâ”€â”€ Tree structure allocation
â”œâ”€â”€ fmap at each bind
â”œâ”€â”€ Interpretation traverses entire tree
â””â”€â”€ Generally 10-100x slower than direct

Optimizations:
â”œâ”€â”€ Church encoding
â”œâ”€â”€ Codensity transformation
â”œâ”€â”€ Reflection without remorse
â””â”€â”€ Fused interpreters
```
...
```
Freer advantages:
â”œâ”€â”€ No Functor constraint
â”œâ”€â”€ More efficient representation
â”œâ”€â”€ Still has interpretation cost

effectful library achievements:
â”œâ”€â”€ Near-native performance
â”œâ”€â”€ Uses IO under the hood
â”œâ”€â”€ Delimited continuations
â””â”€â”€ ~1-5% overhead vs raw IO
```
...
-- Converts O(nÂ²) to O(n) for left-associated binds
...
```haskell
-- Problem: left-associated binds in Free are O(nÂ²)
-- Free fa >>= f >>= g >>= h
...
handle nondet {
    return x â†’ [x]
    choose() k â†’ k(true) ++ k(false)  -- Multiple resumptions!
}
...
StateT s (ExceptT e m)  â‰   ExceptT e (StateT s m)
...
â”œâ”€â”€ Primary: Algebraic effects (B-01 decision)
â”œâ”€â”€ Secondary: MTL-style classes for compatibility
â”œâ”€â”€ Indexed: For security label tracking
â””â”€â”€ Free: For configuration/protocol DSLs
```
...
7. Swierstra, W. (2008). "Data Types Ã  la Carte"

...
### Session A-14: Capability Types — Comparative Analysis
...
| **Concurrency Safe** | Actors | ✓ Native | Partial | Manual | ✓ |
| **Systems Programming** | ✗ | Limited | ✗ | ✓ | N/A |
| **Linear Types** | ✗ | ✗ | ✗ | ✓ | N/A |
| **Effect System** | ✗ | ✗ | ✓ | ✗ | N/A |
...
| Static POLA | Partial | ✓ | ✓ | ✓ | ✓ |
| Information flow | Partial | ✓ | Partial | ✓ | ✓ |
| Data race freedom | Actors | ✓ | Partial | Manual | ✓ |
...
| Bulk revoke | ✓ (subtree) | Membrane | N/A | Region end |
...
Effect Declaration:
  effect FileCapability {
    Open : Path → cap<File>,
    Read : cap<File> → String,
    Write : (cap<File>, String) → (),
    Close : cap<File> → (),
  }
...
Type Rule:
  Γ ⊢ e : cap<R>    Δ, x:cap<R> ⊢ e' : τ ! E ∪ {Use<R>}
  ───────────────────────────────────────────────────────
  Γ, Δ ⊢ withcap x = e in e' : τ ! E
...
letregion ρ in
  letcap c : cap<File> at ρ = open(path) in
    use(c)
  end
end
...
cap<R> at ρ    -- capability in region ρ
&'ρ cap<R>     -- borrow from region

Rules:
  - cap<R> at ρ cannot escape region ρ
...
1. **Linear capabilities optimal for TERAS** — Austral demonstrates viable approach
2. **Effect integration valuable** — Wyvern shows capability-effect synergy
3. **Reference capabilities for concurrency** — Pony model proven
4. **Zero overhead achievable** — Static checking eliminates runtime cost
5. **seL4 complementary** — Kernel + language caps work together
...
Region: Integrated (A-12)
  - cap<R> at ρ for lifetime bounding

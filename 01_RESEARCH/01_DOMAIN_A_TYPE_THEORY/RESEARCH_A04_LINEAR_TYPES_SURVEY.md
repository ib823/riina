...
**Key Insight:** Girard observed that the intuitionistic implication A → B conflates two distinct operations:
1. The pure computational aspect (linear implication A ⊸ B)
...
|--------------|-------------|-------------|
| Linear implication (⊸) | Linear function type | Functions consuming input |
| Tensor (⊗) | Pair type (both) | Parallel composition |
| With (&) | Pair type (choice) | Lazy pairs |
| Plus (⊕) | Sum type | Tagged unions |
...
**Definition:** A linear type A⊸ means values of type A must be used exactly once.
...
Γ, x:A ⊢ M : B
──────────────── (⊸I) [x used exactly once in M]
Γ ⊢ λx.M : A ⊸ B
```

**Context Splitting:** The context Γ, Δ is split between premises; no sharing.
...
**Tensor (⊗) — "Times" or "Par":**
```
A ⊗ B — I have both A and B (simultaneously)
```
...
```
A ⊸ B — Given A (consuming it), I can produce B
```
...
**Par (⅋):**
```
A ⅋ B — Dual of tensor in classical linear logic
```
- "Either A or B, but I don't control which"
- Related to non-determinism

**Multiplicative Units:**
- **1** (one): Unit for ⊗, empty resource
- **⊥** (bottom): Unit for ⅋
...
**With (&) — "With":**
```
A & B — I have both A and B, but can only use one
```
...
**Additive Units:**
- **⊤** (top): Unit for &, any projection succeeds
- **0** (zero): Unit for ⊕, impossible case

### 4.3 Exponential Modalities

**Of Course (!A) — "Bang":**
```
!A — Unlimited supply of A
```
- Can be contracted: !A ⊸ !A ⊗ !A
- Can be weakened: !A ⊸ 1
- Can be derelicted: !A ⊸ A

**Rules:**
```
⊢ Γ, A
──────── (!R) where Γ contains only !-formulas
⊢ Γ, !A
...
**Why Not (?A) — "Question":**
```
?A — Dual of !A in classical linear logic
```
...
|-----------|---------------------|
| A ∧ B | !A ⊗ !B (shared) or A & B (choice) |
| A ∨ B | ?A ⅋ ?B (shared) or A ⊕ B (choice) |
| A → B | !A ⊸ B (unlimited) or A ⊸ B (linear) |
| ¬A | A ⊸ ⊥ (linear) or A⊥ (involution) |
...
|----------|-------------|----------|
| Positive | ⊗, ⊕, 1, 0, ! | Invertible on left |
| Negative | ⅋, &, ⊥, ⊤, ? | Invertible on right |
...
**Closure:** Internal hom A ⊸ B with adjunction:
```
Hom(A ⊗ B, C) ≅ Hom(A, B ⊸ C)
```
...
| Property | CCC (λ-calculus) | SMCC (Linear) |
|----------|-----------------|---------------|
| Product | A × B (diagonal Δ exists) | A ⊗ B (no diagonal) |
| Diagonal | Δ : A → A × A | None |
| Terminal | 1 with ! : A → 1 | I (no general map) |
| Copy | Free | Requires ! |
| Discard | Free | Requires affine |

**Key Insight:** The diagonal morphism Δ : A → A × A enables copying in CCCs. SMCCs lack this, enforcing linearity.
...
- SMCC structure
- Dualizing object ⊥
- Isomorphism A ≅ (A ⊸ ⊥) ⊸ ⊥ (double negation)
...
- δ : !A → !!A (comultiplication, digging)
...
**Chu(C, ⊥)** provides a canonical way to build *-autonomous categories:

- Objects: pairs (A⁺, A⁻) with pairing A⁺ × A⁻ → ⊥
...
**Type System:**
```
τ ::= bool | unit | &ρ mut? τ | Box<τ> | ...
```

**Place Typing:**
```
Γ ⊢ π : τ @ ρ    (place π has type τ in region ρ)
```
...
Δ; Γ ⊢ e : τ ⊣ Δ'
```
- Δ: loan set (active borrows)
- Γ: variable environment
- Δ': updated loans after evaluation
...
**Example:**
```
{x: File | isOpen(x)} ⊸ {y: File | isClosed(y)}
```
...
derive_key : MasterKey ⊸ (SessionKey ⊗ MasterKey')
encrypt : SessionKey ⊗ Plaintext ⊸ Ciphertext
decrypt : SessionKey ⊗ Ciphertext ⊸ Plaintext ⊗ SessionKey
destroy : SessionKey ⊸ unit
```
...
access : Cap[R] ⊸ (R ⊗ Cap[R])  // use and return
revoke : Cap[R] ⊸ unit           // capability consumed
```
...
**Linear Channels:**
```
send : Chan[!A.S] ⊗ A ⊸ Chan[S]
recv : Chan[?A.S] ⊸ (A ⊗ Chan[S])
close : Chan[end] ⊸ unit
```
...
log_action : Action → AuditEntry
commit : AuditEntry → IO unit  // ensures entry is recorded
```
...
```
type SecureKey{l:Label} = {k: Key | level(k) = l} ⊸
```

**With Dependent Types (A-01):**
```
type Vec[A, n:Nat] = A^n ⊸
```

**With Effect System:**
```
read_key : () → IO[KeyStore] Key ⊸
```
...
10. Mazurak, K. et al. (2010). "Lightweight Linear Types in System F°." TLDI 2010.
...
19. DeLine, R. & Fähndrich, M. (2001). "Enforcing High-Level Protocols in Low-Level Software." PLDI 2001.
...
22. Mellies, P.-A. (2009). "Categorical Semantics of Linear Logic." Panoramas et Synthèses.

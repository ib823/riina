...
### Session A-16: Sized Types — Comparative Analysis
...
| Feature | Agda Sized | F* Decreases | MiniAgda | CIC^ω |
|---------|------------|--------------|----------|-------|
...
| Property | Agda | F* | MiniAgda | CIC^ω |
|----------|------|----|---------:|-------|
| Structural recursion | ✓ | ✓ | ✓ | ✓ |
| Sized types | ✓ | Via measures | ✓ | ✓ |
| Lexicographic | ✓ | ✓ | Limited | ✓ |
| Custom measures | Limited | ✓ | ✗ | ✗ |
| Mutual recursion | ✓ | ✓ | ✓ | ✓ |
...
CIC^ω: Formal foundations, ordinal theory
```
...
length : {A : Set} {i : Size} → List A i → Nat
length [] = 0
...
With Sized Types:
  ✓ merge : List^i A → List^j A → List^(i+j) A
...
Option 2: Linear Size Consumption
  ...
  Example:
    bounded_use : lin Size^i → Resource → Result
...
Example:
  consume_bounded : ∀i. lin Buffer^i → Result
...
Usage Pattern:
  call_api : cap<API, Call>^(↑i) → Req → (Resp, cap<API, Call>^i)
...
code[A^i → B]    -- generated function terminates on size-i input

Example:
  compile_algo : ∀i. Algorithm → code[Data^i → Result]
  -- Compiled algorithm terminates on bounded input

Staging with Bounds:
  specialize : ∀i. Pattern → code[Input^i → MatchResult]
...
1. **Agda-style provides best balance** — practical inference with good guarantees
2. **Structural recursion often sufficient** — sized types for edge cases
3. **Zero runtime overhead universal** — all systems erase sizes
4. **Integration feasible** — can combine with linear/effect/capability types
5. **F* measures most expressive** — but manual annotation required

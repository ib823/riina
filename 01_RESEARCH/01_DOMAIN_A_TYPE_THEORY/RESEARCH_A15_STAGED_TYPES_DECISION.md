...
### Session A-15: Staged Types — Architecture Decision Record
...
code[τ]          -- code producing value of type τ
code[τ ! E]      -- code with effect E when run
lin code[τ]      -- linear code (must run exactly once)
code[τ] at ρ     -- code in region ρ
...
Escape (splice):
  ...
  Typing:
    Γ₀ ⊢ e : code[τ]   (in bracket context Γ₁)
    ─────────────────────────────────────────────
    Γ₀ ⊗ Γ₁ ⊢ .~e : τ
...
Stage Levels:
  n ∈ ℕ           -- stage level (0 = current, 1 = next, ...)

Classifier Annotations:
  Γⁿ              -- context at stage n
  τⁿ              -- type at stage n
...
Typing:
  comptime N: τ   -- compile-time parameter
...
Use Cases:
  ...
  code[τ] -- specialization, DSL compilation, metaprogramming
```
...
Linearity Modes for Code:
  lin code[τ]     -- code must be run exactly once
  aff code[τ]     -- code may be run at most once
  unr code[τ]     -- code may be run multiple times (default)

Linear Results:
  code[lin τ]     -- code produces linear value
  
Combined:
  lin code[lin τ] -- linear code producing linear value

Typing Rules:
  Γ ⊢ e : lin code[τ]    e used exactly once
  ────────────────────────────────────────────
  Γ ⊢ run e : τ
...
Effect in Generated Code:
  code[τ ! E]     -- running code has effect E
...
Effect During Staging:
  (code[τ]) ! F   -- constructing code has effect F
...
Code in Region:
  code[τ] at ρ    -- code value lives in region ρ
  
  letregion ρ in
      let c : code[Data] at ρ = .<compute()>.
      let result = run c
  end  // code deallocated with region

Cross-Stage Region Safety:
  .<letregion ρ in
      let x : Data at ρ = allocate()
...
Region Effects in Code:
  code[τ ! Alloc<ρ>]   -- code allocates in region ρ
```
...
THEOREM (Stage Preservation):
  If Γ ⊢ e : code[τ] then run e : τ is well-typed.
  (Generated code preserves types)

THEOREM (Scope Safety):
  If Γⁿ ⊢ .<e>. : code[τ] then e does not reference
...
THEOREM (Linear Staging):
  If c : lin code[τ], then c is run exactly once.
  (Linear code discipline preserved)

THEOREM (Effect Safety):
  If e : code[τ ! E], then run e : τ ! E.
...
1. **Core Staging:** code[τ] types with bracket/escape/run
...
3. **Integration:** Full coherence with prior decisions
   - A-04: lin code[τ] for linear staging
   - A-11: code[τ ! E] for effectful code
...
|-------------|------------|-------|
| D38 (Performance) | ✓ Full | Specialization enables optimization |
| Type Safety | ✓ Full | Environment classifiers |
| Integration | ✓ Full | Linear, effects, capabilities |
| Security | ✓ Full | Scope safety, hygiene |
...
compile_format : Format -> Type × code[... -> String]
...
Γⁿ ⊢ e : τ        -- e has type τ at stage n
Γ ⊢ e ⇓ v         -- e evaluates to v
⟦e⟧ = code        -- code generation
```

...
**Description**: Value-dependent protocols à la Toninho-Caires-Pfenning.
...
session S ::=
    ...
    | +{l₁:S₁, ...}  // Internal choice (select)
    | &{l₁:S₁, ...}  // External choice (offer)
...
Where:
  Γ = Standard context (unrestricted)
  Δ = Session context (linear)
  T = Result type
  Δ' = Remaining session context
```
...
∀i. Γ; Δ, c:Sᵢ ⊢ eᵢ : T; Δ'
────────────────────────────────────────────────
Γ; Δ, c:&{lᵢ:Sᵢ} ⊢ offer(c){lᵢ → eᵢ} : T; Δ'
```

### 4.3 Refinement Integration

```
Γ ⊢ v : T    {φ}    Γ; Δ, c:S ⊢ e : U; Δ'
────────────────────────────────────────────────
Γ; Δ, c:!{x:T|φ}.S ⊢ send(c, v); e : U; Δ'
                    when SMT(φ[v/x])
```

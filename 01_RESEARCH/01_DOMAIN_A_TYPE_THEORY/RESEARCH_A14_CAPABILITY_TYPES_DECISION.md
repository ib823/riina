...
### Session A-14: Capability Types — Architecture Decision
...
P4: Attenuation
    Can restrict permissions: P → P' where P' ⊆ P
    Cannot amplify: never P → P' where P' ⊃ P
...
With Region (A-12):
  cap<R, P> at ρ            -- capability in region
```
...
Γ ⊢ c : cap<Sys, Create<R>>
────────────────────────────
Γ ⊢ create(c) : cap<R, Full>
```
...
// Type rule
Γ ⊢ c : cap<R, P>    use_op ∈ P
──────────────────────────────
Γ ⊢ c.use_op() : τ ! {R.use_op}
```
...
Γ ⊢ c : lin cap<R, P>    Δ, x : lin cap<R, P> ⊢ e : τ
─────────────────────────────────────────────────────
...
letregion ρ in
    letcap f: cap<File, Full> at ρ = open("data.txt") in
        let content = f.read();
...
// Type with region
cap<File, Read> at ρ    -- file cap in region ρ
&'ρ cap<File, Read>     -- borrow scoped to region
...
Formation Rules:
  ...
  R resource    P permission    ρ region
  ───────────────────────────────────────  (cap-region-form)
  cap<R, P> at ρ type
```
...
| A-12 Regions | cap<R, P> at ρ | High |
...
7. **Region Integration**: `cap<R, P> at ρ` for lifetime bounding
...
**Next Session:** A-15 — Staged Types (Multi-Stage Programming)

...
Where:
  usage ∈ {0, 1, ω}     -- from A-04
  uniqueness ∈ {shared, unique}
```
...
```
Grading dimensions:
  Multiplicity m ∈ {0, 1, ω, affine, relevant}  -- from A-04
  Uniqueness u ∈ {shared, unique}               -- from A-06

Type form: T @ (m, u)
Shorthand:
  lin T     ≡ T @ (1, shared)
  uniq T    ≡ T @ (ω, unique)  
  lin uniq T ≡ T @ (1, unique)
```
...
unique → shared  (OK: can forget uniqueness)
shared → unique  (ERROR: cannot prove uniqueness)
```
...
Type ::= 
    ...
    | uniq Type          -- sugar for T @ (ω, unique)
...
| ω | shared | Unrestricted, possibly shared |
| ω | unique | Unrestricted, definitely unique |
...
Extended borrow state:
  Location → (Type, Ownership, Uniqueness, Borrows)
...
3. **Refinement proof**: Functional ≤ Imperative
...
| A-06-D6 | unique → shared coercion | Safe subsumption |

...
│  - No `?` type allowed in core code                        │
│  - Linear types, effects, refinements all static           │
└─────────────────────────┬───────────────────────────────────┘
                          │
              ┌───────────┴───────────┐
              │    TRUST BOUNDARY     │
...
│                     (Untyped)                               │
│                                                             │
│  - C libraries, plugins, FFI, network data                 │
│  - No type guarantees from TERAS perspective               │
└─────────────────────────────────────────────────────────────┘
```
...
?_ω : BoundaryType    -- Unrestricted unknown (default)
...
τ ⊑ τ           -- Reflexivity
Int ⊑ Int       -- Same type equally precise

-- Function types (contravariant in input)
(τ₁ → τ₂) ⊑ (τ₁' → τ₂')  iff  τ₁' ⊑ τ₁  and  τ₂ ⊑ τ₂'
```

**Guarantee Statement:**
> If typed boundary code e₁ is refined to more precise types e₂ (e₁ ⊑ e₂),
> and both type check, then:
> - e₂ produces the same results as e₁, OR
> - e₂ fails at compile time (more restrictive), OR  
> - e₂ produces a blame error earlier than e₁
...
If:
- `Γ ⊢_core e : τ` (e is well-typed core code)

...
This creates a lattice of type refinements where α list is at top, with refinements like α singleton ∨ α nil below it.
...
```
{ν : B | P(ν)}     -- Base type B with predicate P
x:τ₁ → τ₂          -- Dependent function type
```

**Liquid Type Variable κ**:
...
- `τ⇓` (Fin-type): Finite values, guaranteed to terminate
- `τ↓` (Div-type): May diverge but reduces to value if terminates
- `τ` : Arbitrary expressions
...
| DML | SML | Linear arith | — | Pure | Native |

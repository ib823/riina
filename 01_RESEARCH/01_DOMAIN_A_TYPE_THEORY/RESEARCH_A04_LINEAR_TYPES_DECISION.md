...
3. **Graded multiplicity annotations** (0, 1, ω) for fine-grained resource control
...
Multiplicities: m ::= 0        -- erased (type-level only)
                    ...
                    | ω        -- unrestricted (any usage)
                    | ≤1       -- affine (at most once) [default]
                    | ≥1       -- relevant (at least once)
```
...
```teras
// Refined linear type
type SecretKey = lin {k: Key | security_level(k) ≥ Secret}

// Refined affine capability
type FileHandle = {h: Handle | valid(h) ∧ mode(h) = ReadOnly}
...
Types with Multiplicity:
  τ ::= ...
      | m τ            -- type with multiplicity m
```

**Multiplicity Arithmetic:**
```
0 + m = m
1 + 1 = ω       (two uses = unlimited)
ω + m = ω
0 × m = 0
1 × m = m
ω × m = ω (if m ≠ 0), 0 (if m = 0)
```

**Subtyping:**
```
0 ≤ 1 ≤ ω      (0 most restrictive, ω least)
```
...
- Γ: Input typing context with multiplicities
- e: Expression
- τ: Type (may include multiplicity)
- Γ': Output context (tracking consumed resources)
...
```
              ...
              x fresh, unique(e)
BorrowMut:    ──────────────────────────────────────────
              Γ ⊢ &mut e : &'a mut τ ⫤ Γ', x : borrowed_mut(τ, 'a)
```
...
pub type AuthToken = lin {
    token: TokenData |
    not_used(token) ∧ not_expired(token)
}
...
├── src/
│   └── typecheck/
│       ├── context.rs       -- Typing context with multiplicities
│       ├── linearity.rs     -- Linearity checking
│       ├── ownership.rs     -- Ownership tracking
│       ├── borrowck.rs      -- Borrow checker
│       ├── lifetime.rs      -- Lifetime inference and checking
│       └── integration.rs   -- Integration with refinement types
```
...
```
              ...
              ▼              ▼              ▼
    ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
    │ Linearity    │ │ Refinement   │ │ Dependent    │
...
- [ ] Multiplicity annotations (0, 1, ω)
- [ ] Linear (lin) marker
- [ ] Relevant (≥1) support
...
|-----------|-----------|-------|
| Law 1: Self-Sufficiency | ✓ | No GC or runtime dependencies |
| Law 2: Formal Verification | ✓ | Type system provides proofs |
| Law 3: Zero Third-Party | ✓ | Custom implementation |
| Law 4: Malaysian Identity | ○ | Neutral (type system) |
| Law 5: Post-Quantum Ready | ○ | Orthogonal concern |
| Law 6: Memory Safety | ✓ | Core guarantee |
| Law 7: Auditability | ✓ | Clear ownership semantics |

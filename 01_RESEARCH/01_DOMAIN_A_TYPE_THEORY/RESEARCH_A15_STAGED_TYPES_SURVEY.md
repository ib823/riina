...
Code Types:
  <τ>   -- code of type τ (to be executed later)
...
Typing Rules:
  ...
  Γ₀ ⊢ e : τ     (in code context Γ₁)
  ──────────────  (escape)
  Γ₀, Γ₁ ⊢ ~e : τ  (splice into code)
...
Escape: Splice into bracket
  ~e     : τ       (if e : code τ, inside bracket)
  
Run: Execute code
  !e     : τ       (if e : code τ)
  
Lift: Convert value to code
  lift e : code τ  (if e : τ, τ liftable)
...
Types:
  τ ::= int | bool | τ → τ | code τ | ...

Contexts:
  Γ ::= · | Γ, x:τ
  
Level Annotation:
  Γ^n denotes context at level n
...
Quotation:
  ...
  [t| τ |]     : Q Type      (type quote)
  [p| pat |]   : Q Pat       (pattern quote)
  
Splice:
  $(e)         : τ           (if e : Q Exp, result : τ)
...
Typed Quotation:
  [|| e ||]    : Q (TExp τ)  (typed expression quote)
...
S4 Modal Logic:
  □A    -- "necessarily A" = code of type A
  ◇A    -- "possibly A" = (not typically used)
...
Modal Validity:
  ...
  Γ = valid hypotheses (usable at any stage)
  Δ = hypotheses at current stage only
```
...
Solution: Track environment in type
  <A>^ρ   -- code of type A, valid in environment ρ
  
Classifiers:
  ρ ::= ε          -- empty (closed code)
       | ρ, x:A    -- extended environment
...
Consistency Rules:
  - Variables at level n can only be used at level ≥ n
  - Brackets increase level by 1
  - Escapes decrease level by 1
  - Run requires level 0
  
Example:
  Γ^0 ⊢ x : int                    -- x at level 0
  Γ^0 ⊢ <x + 1> : <int>            -- code at level 0
  Γ^1 ⊢ x + 1 : int                -- inside bracket
  Γ^0 ⊢ !<x + 1> : int             -- run at level 0
```
...
Includes reduction rules:
  !<e> → e                (run bracket)
  <~e> → e                (bracket escape, if e : <τ>)
  
Code Reduction:
  <e₁> ⊕ <e₂> → <e₁ ⊕ e₂>  (combine code)
```
...
Solution: Level checking
  ...
  Γ^n ⊢ y : τ requires y ∈ Γ^m for some m ≤ n
  
  - Variables must be bound at level ≤ use level
...
lin <τ>    -- linear code (must be run exactly once)
...
<τ ! E>    -- code with effect E when run

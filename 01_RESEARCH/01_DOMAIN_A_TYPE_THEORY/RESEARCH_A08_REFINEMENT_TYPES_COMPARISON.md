...
| Extraction | Native Haskell | C, OCaml, F#, JS | Native SML | Native Rust | — |

### 1.2 Type System Power

| Capability | Liquid Haskell | F* | DML | Flux |
|------------|---------------|-----|-----|------|
| Predicate subtypes | ✓ | ✓ | ✓ (index) | ✓ |
| Dependent functions | ✓ | ✓ | ✓ | ✓ |
| Abstract refinements | ✓ | ✓ | — | ✓ |
| Measures | ✓ | ✓ | Built-in | ✓ |
| Refinement reflection | ✓ | — | — | — |
| Higher-order refinements | Limited | ✓ | — | Limited |
...
```fstar
val sort : l:list int -> l':list int{sorted l' ∧ permutation l l'}
```
...
```haskell
τ⇓  -- Terminating values (refinements hold)
τ↓  -- May diverge (refinements conditional)
τ   -- Arbitrary (no refinement guarantees)
```
...
|--------|-------------|-----------------|----------------|
| Liquid Haskell | ✓ (via length) | N/A (GC) | N/A (GC) |
| F*/Low* | ✓ (via preconditions) | ✓ | ✓ |
| DML | ✓ (via indexes) | N/A | N/A |
| Flux | ✓ (via ownership + refinement) | ✓ (Rust) | ✓ (Rust) |
...
- Liquid Haskell → Native Haskell (GHC)
- F* → C/OCaml (native performance)
- DML → Native SML
- Flux → Native Rust
...
| Decidable Checking | ✓ | Partial | ✓ | ✓ |
| Low-Level Control | — | High (Low*) | — | High |
| Effect Tracking | Partial | ✓ | — | Via ownership |

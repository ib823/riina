...
⟨Int ⇐ ?⟩ v = if tag(v) == INT then v else error
```
- Shallow checks only
- Low overhead
- Less precise blame
...
|----------|----------------|------------------|
| Secret → ? | Runtime label check | Label lost |
| ? → Public | Runtime verification | No check |
| ? → ? | Propagate dynamically | Unknown |
...
(1 x: ?) → ?    -- OK: linear unknown
(? x: Int) → ?  -- ERROR: quantity must be explicit
```
...
(x: Int) → ?{x}    -- Result depends on x somehow
...
impl PluginAPI for untrusted_plugin {
  fn transform(data: Public<Data>) -> Result<Public<Data>, Error> {
    ...
    ⟨Public<Data> ⇐^untrusted_plugin ?⟩ 
      untrusted_plugin.transform(
        ⟨? ⇐^untrusted_plugin Public<Data>⟩ data
      )
  }
}
```

...
### Session A-13: Ownership Types — Comparative Analysis
...
|---------|------|-------|---------|------|----------------|---------|------|
| **Static Safety** | ✓ Full | ✓ Full | ✓ Full | ✓ Full | ✓ Full | ✓ Full | Hybrid |
| **Move Semantics** | ✓ | ✓ | Partial | ✓ | ✓ | ✓ | ✓ |
| **Borrowing** | ✓ Rich | ✓ Focus | ✓ Regions | ✗ | ✗ | ✓ | ✓ |
| **Interior Mutability** | ✓ | ✓ | ✗ | ✗ | ✗ | ✓ | ✓ |
| **Permission Annotations** | Implicit | Explicit | Explicit | Explicit | Multiplicity | Explicit | Mixed |
| **Adoption/Focus** | ✗ | ✓ | ✗ | ✗ | ✗ | ✗ | ✗ |
| **Lifetime Tracking** | ✓ | Via regions | ✓ | ✗ | ✗ | ✓ | Runtime |
| **Production Ready** | ✓ | Research | Ended | Research | ✓ (GHC 9+) | ✓ | Research |
| **Systems Programming** | ✓ | ✗ | ✓ | ✗ | ✗ | ✓ | ✓ |
| **Generic Ownership** | ✓ | ✓ | Partial | ✓ | ✓ | ✓ | ✓ |
...
```
Cyclone (2001-2006) → Influenced → Rust (2010+)

Key inheritances:
  Cyclone *`U unique   →  Rust Box<T>
  Cyclone regions      →  Rust lifetimes
  Cyclone fat pointers →  Rust slices
  Cyclone NULL checks  →  Rust Option<T>
```
...
owned T      ≡  aff T + Drop      // can discard
borrowed T   ≡  lin ref T         // must return
moved T      ≡  lin T             // must consume
...
```
Rust lifetimes ≈ Region types:

'a in Rust    ≈  ρ in MLKit
&'a T         ≈  τ at ρ
'a: 'b        ≈  ρ₁ ⊇ ρ₂ (outlives)

TERAS-LANG Integration:
  owned T at ρ    -- owned value in region
  &'ρ T           -- borrow from region
  &'ρ mut T       -- mutable borrow from region
```
...
letregion ρ with cap in
  let x: owned T at ρ = alloc<ρ>(cap, value);
  // x owned, in region ρ
...
Specific guarantees:
  ...
  No double-free:
    ∀ ptr. free(ptr) ⇒ owned(ptr) ∧ unique(ptr)
    
  No data races:
    ∀ x. writing(t₁, x) ⇒ ¬accessing(t₂, x) for t₁ ≠ t₂
```
...
1. **Rust dominates practical ownership** — Proven at scale, zero overhead, complete safety
2. **Mezzo offers theoretical clarity** — Explicit permissions, adoption/focus flexibility
3. **Austral validates modern design** — Clean-slate linear types work for systems
4. **Vale shows hybrid potential** — Runtime checks enable flexibility when needed
5. **Linear Haskell proves FP integration** — Retrofitting linearity is possible

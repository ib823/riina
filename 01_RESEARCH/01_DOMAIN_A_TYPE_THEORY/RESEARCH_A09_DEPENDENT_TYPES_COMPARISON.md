...
This document compares three major dependently typed languages—Agda, Idris 2, and Lean 4—for their applicability to TERAS-LANG design. Each language represents different trade-offs between theoretical purity, practical programming, and mathematical verification.
...
|---------|------|---------|--------|----------------|
| **Linear Types** | ✗ | ✓ (QTT) | ✗ | Critical (resource safety) |
| **Algebraic Effects** | ✗ | ✓ | ✗ | High (controlled IO) |
| **Proof Irrelevance** | Partial | ✓ | ✓ | Medium (efficiency) |
| **Universe Polymorphism** | ✓ | ✓ | ✓ | Medium |
...
**Key Difference:** Idris 2 can make vector construction linear, ensuring elements are used exactly once—critical for secure buffer handling.
...
bind : Labeled l A → (A → Labeled l B) → Labeled l B
```
...
```lean
-- Indexed monad
structure Labeled (l : Label) (α : Type) where
  value : α

def bind (x : Labeled l α) (f : α → Labeled l β) : Labeled l β :=
  ⟨f x.value |>.value⟩
```
...
**Agda:** Relies on Haskell runtime GC—no memory safety guarantees at language level.
...
def withBuffer (size : Nat) (f : Buffer → IO α) : IO α := do
  let buf ← Buffer.alloc size
  let result ← f buf
  -- buf automatically freed when refcount → 0
  pure result
```
...
|---------------|----------------|--------|
| Linearity | QTT-based (0, 1, ω) | Idris 2 |

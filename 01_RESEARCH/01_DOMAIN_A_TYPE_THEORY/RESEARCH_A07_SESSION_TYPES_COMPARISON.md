...
| **Duality** | Built-in (S and S̄) | Implicit in global type |
...
### 2.2 Comparison with π-calculus

| Aspect | Session Types | π-calculus |
|--------|---------------|------------|
...
|--------|----------------|---------------|
| **Type equality** | μX.S = S[X/μX.S] | Explicit fold/unfold |
...
|------------|-----------------|-------------|-------|
| A ⊗ B | Send A, continue B | Parallel output | |
| A ⅋ B | Receive A, continue B | Parallel input | |
| A ⊕ B | Internal choice | Output choice | |
...
| ProVerif | Applied π | Symbolic | Semi-automatic |
...
|---------|----------|--------|------------|-------|
| session-types | Indexed types | ✓ | ✗ | Mature |
| sesstype | Affine types | ✓ | ✗ | Simple |
| sesh | Effect-based | ✓ | ✗ | Experimental |

### 8.2 Functional Language Implementations

| Language | Implementation | Static | Linear | Notes |
|----------|----------------|--------|--------|-------|
| Links | Native | ✓ | ✓ | Research |
| Haskell | Library | ✓ | Encoded | Multiple libs |
| OCaml | Effects | ✓ | ✗ | OCaml 5 |
| Idris | Native | ✓ | ✓ | Dependent |
| Granule | Native | ✓ | ✓ | Graded |
...
```
TERAS Session Types:
├── Binary sessions (core)
│   ├── Linear endpoint ownership
│   ├── Duality checking
│   └── Refinement predicates
├── Multiparty sessions (extension)
│   ├── Global types
│   ├── Projection
│   └── Protocol DSL
└── Dependent sessions (future)
    └── Value-dependent protocols
```

...
### Session A-15: Staged Types — Comparative Analysis
...
|----------|-----------|------------------|---------|------|-----|
| Well-typed generation | ✓ Full | TExp only | ✓ Full | ✗ | Limited |
| Scope extrusion prevention | ✓ | ✓ | ✓ | Manual | ✓ |
| Phase distinction | ✓ Full | Partial | ✓ Full | ✗ | ✓ |
...
Option 2: Staged Linear Values
  code[lin τ]    -- code producing linear value
...
lin code[τ]     -- code itself is linear (run once)
code[lin τ]     -- code produces linear result
lin code[lin τ] -- both
...
Option 2: Effect During Staging
  (code[τ]) ! E  -- effect during code construction
...
code[τ ! E]       -- runtime effect E
(code[τ]) ! F     -- staging-time effect F
(code[τ ! E]) ! F -- both
...
code[τ] at ρ     -- code valid in region ρ
...
3. Dead Code Elimination
   ...
   →  only A or B compiled
   
4. Type Specialization
   generic<T>(x)  →  specialized_int(x)
```
...
1. **MetaOCaml provides strongest type safety** — environment classifiers and scope safety
2. **Scala 3 offers good balance** — typed quotes with practical macro system
3. **Compile-time evaluation valuable** — Zig comptime for simple cases
4. **Rust proc_macro insufficient** — lacks type safety for security-critical code
5. **Integration feasible** — can combine with linear types, effects, capabilities
...
Integration:
  - lin code[τ] for linear staged code
  - code[τ ! E] for effectful generated code

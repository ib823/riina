...
Graph node with unique type:
    ┌─────────────────┐
    │  Node (value)   │  ← exactly one reference
    └─────────────────┘

Graph node with shared type:
    ┌─────────────────┐
    │  Node (value)   │  ← multiple references
    └─────────────────┘
         ↑     ↑
       ref1   ref2
```
...
```
*T ≤ T    (unique can become non-unique)
T ≰ *T    (non-unique cannot become unique)
```
...
5. **Coercion**: unique → affine → unrestricted
...
4. **Uniqueness ≈ affine** but with reference-counting perspective

...
Properties:
├── Unforgeable: Cannot be created out of thin air
├── Transferable: Can be passed to others
├── Attenuable: Can create weaker capabilities
├── First-class: Can be stored, passed, returned
└── Specific: Grants access to specific resource
```
...
Access Control Lists (ACL):
    Resource → Set<Principal>
    "Who can access this resource?"

Capabilities:
    Principal → Set<Capability>
...
```
Timeline:
1966    │ Dennis & Van Horn: Capability concept
1974    │ Hydra: Capability-based OS
1979    │ Capability-Based Computer Systems (Levy)
1988    │ KeyKOS: Persistent capabilities
1997    │ E Language: Object capabilities
2003    │ Caja: Capability-safe JavaScript
2010    │ Capsicum: FreeBSD capabilities
2015    │ CHERI: Hardware capabilities
```
...
Features:
├── Fat pointers (128/256 bits)
├── Hardware-enforced bounds
├── Permission bits in pointers
...
┌────────────────────────────────────────────────────────────┐
│ Permissions │ Object Type │ Bounds │ Base Address │ Cursor │
│   (16 bits) │   (24 bits) │(varies)│   (varies)   │(varies)│
└────────────────────────────────────────────────────────────┘
...
1. God capability
   ✗ struct AllAccess { /* everything */ }
   ✓ Separate capabilities per resource

2. Implicit capability from context
   ✗ fn delete() { ambient_fs().delete(current_file()) }
   ✓ fn delete(fs: FsCap, file: FileCap) { fs.delete(file) }

3. Capability leak
   ✗ static CACHED_CAP: Cap = ...
   ✓ Pass capabilities through call stack

4. Missing attenuation
   ✗ pass(my_full_cap)
   ✓ pass(my_cap.read_only())
```
...
*Research Track Output — Domain D: Capability-Based Security*

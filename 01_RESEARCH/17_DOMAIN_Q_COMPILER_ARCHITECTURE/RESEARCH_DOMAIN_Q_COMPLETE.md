...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    TERAS Compiler Pipeline                          │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Source → Lexer → Parser → AST → Type Checker → MIR → Optimizer    │
│                                        ↓                            │
│                              Security Checker                       │
│                                        ↓                            │
...
Phases:
1. Frontend: Source → AST (parsing, desugaring)
2. Middle: AST → MIR (type checking, security analysis)
3. Backend: MIR → Binary (optimization, code generation)
```
...
impl IFCChecker {
    ...
    fn check_assignment(&mut self, target: &Ident, source: &TypedExpr) -> Result<(), IFCError> {
        ...
        // Check: source_label ⊔ pc_label ⊑ target_label
...
*Research Track Output — Domain Q: Compiler Architecture and Implementation*

...
| COE-05 | Integration with effects | Unified A -[ε]-> B @ r syntax |
...
fn copy_file(src: &str, dst: &str) -> () @ {FileRead, FileWrite} {
    // Coeffects combine: {FileRead} ∪ {FileWrite}
...
fn process_mixed() -> Result @ Clearance::Secret {
    // Clearance is join: Public ⊔ Secret = Secret
...
fn process_request() -> Response @ Provenance::{UserInput, Database} {
    // Provenance union: {UserInput} ∪ {Database}
...
where
    UserInput ∉ P  // No user input allowed!
{

...
impl EffectCategory {
    ...
    pub fn from_effect(eff: riina_types::Effect) -> Option<Self> {
        ...
        match eff {
            ...
            | Effect::Sandi
            | Effect::Menara
            | Effect::Gapura => Some(Self::Product),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn union_works() {
        let a = EffectPermissions {
            io: true,
            ..Default::default()
        };
        let b = EffectPermissions {
            crypto: true,
            ..Default::default()
        };
        let c = a.union(&b);
    ...
    fn escalation_detected() {
        let parent = EffectPermissions {
            io: true,
            ..Default::default()
        };
        let dep = EffectPermissions {
            io: true,
            network: true,
            ..Default::default()
        };
    ...
    fn from_effect_mapping() {
        ...
        assert_eq!(
            EffectCategory::from_effect(Effect::FileSystem),
            Some(EffectCategory::IO)
        );
        assert_eq!(
            EffectCategory::from_effect(Effect::Network),
            Some(EffectCategory::Network)
        );
        assert_eq!(
            EffectCategory::from_effect(Effect::Crypto),
            Some(EffectCategory::Crypto)
        );
        assert_eq!(
            EffectCategory::from_effect(Effect::Process),
            Some(EffectCategory::System)
        );
        assert_eq!(
            EffectCategory::from_effect(Effect::Panel),
            Some(EffectCategory::Product)
        );
    }
}
